...
def build_chains(llm1, llm2, retriever, debug: bool = False, existing_memory: ConversationBufferWindowMemory | None = None):
    ...
    prompt1 = PromptTemplate(
        ...
        template=(
            ...
            "- Abschnittstitel ausschließlich als **fette Überschriften** (Markdown-Syntax `**Titel**`). *Keine kursiven Überschriften.*\n"
            "- Nach jeder Überschrift GENAU EINE LEERE ZEILE.\n"
            "- Inhalte unter Überschriften als Markdown-Liste (\"- \").\n"
            "- Keine Preise oder Marken. Am Ende der Schätzung stelle explizit die Frage: \"Passen die Mengen so oder wünschen Sie Änderungen?\"\n"
            ...
            "## Ausgabe-Logik\n"
            "- *Schätzung*: Gliedere in Projektbeschreibung → Leistungsdaten → Materialbedarf → Transparenz. Frage am Ende explizit, ob die Mengen so passen oder geändert werden sollen, und hänge den Maschinenanhang an.\n"
