...
def extract_products_from_output(output: str) -> List[str]:
    ...
    for line in output.splitlines():
        ...
        if s.startswith(('•','–','-')) and len(s)>3:
            cl=s.lstrip('•–-').strip()
            if cl and not any(b in cl.lower() for b in bad): out.append(cl); continue
        if ':' in s and not any(b in s.lower() for b in bad):
            left,right=s.split(':',1)
            if len(left.strip())>3 and right.strip(): out.append(s)
    return out
...
def parse_positions(llm2_json: str)->List[Dict[str,Any]]:
    ...
    if not isinstance(raw, list): raise ValueError("Erwarte JSON-Array")
    out=[]
    for i,pos in enumerate(raw,1):
        name=str(pos.get("name","")).strip()
        einheit=str(pos.get("einheit","")).strip()
        try:
            menge=float(pos.get("menge",0)); epreis=float(pos.get("epreis",0))
        except: continue
        if not name or not einheit or menge<=0 or epreis<0: continue
        gesamt=round(menge*epreis,2)
        out.append({
            "nr":i, "name":name,
            "menge": int(menge) if float(menge).is_integer() else menge,
            "einheit":einheit, "epreis":round(epreis,2),
            "gesamtpreis": gesamt
        })
    return out
