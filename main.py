...
def api_offer(payload: Dict[str, Any] = Body(...)):
    ...
    # DOCUMENTS: List[Document] mit .page_content == voller Katalognamen-Zeile
    catalog_lines = [ (d.page_content or "").strip() for d in DOCUMENTS ]
    catalog_lower = { line.lower(): line for line in catalog_lines }  # map lower->original

    def find_exact_catalog_lines(terms: list[str]) -> list[str]:
        """Versucht exakte Matches (case-insensitive) gegen Katalognamen.
        Fällt auf Retriever zurück, wenn kein 1:1 Match existiert."""
        ctx = []
        seen = set()
        # 1) exakte (case-insensitive) Treffer
        for t in terms:
        ...
        # 2) Fallback pro term mit Retriever (falls nicht exakt gefunden)
        for t in terms:
            key = (t or "").strip().lower()
            if not key or key in catalog_lower:
                continue
            # Mehr Kandidaten zulassen, damit der exakte String eher vorkommt
    ...
    # --- Falls der Frontend-Input 'message' schon eine Materialien-Liste enthält, nutze diese ---
    # Das macht Produkte aus dem Nutzer-Input "stärker" als die Assistenten-Antwort.
    products_from_message = extract_products_from_output(message) if message else []

    # Bestimme finale Produktliste-Quelle:
    # 1) payload.products[] explizit
    # 2) message (Materialien: ... )
    # 3) Chat-Verlauf (Assistenten-Output) – letzter Ausweg
    if products and isinstance(products, list) and all(isinstance(x, str) for x in products):
        ...
        # Fallback: aus Chat-Kontext extrahieren
    ...
    # --- Kontext konservativ aufbauen: erst exakte Katalogzeilen, dann Fallback ---
    ctx_lines = find_exact_catalog_lines(chosen_products)
    if not ctx_lines:
        # Nichts brauchbares gefunden -> leere Antwort
        return {"positions": [], "raw": "[]"}

    # Kontext deckeln (gegen Token-Explosion)
    chunks = []
    total_chars = 0
    ...
    # Prompt2 DIREKT füttern (keine chain2-Heuristik mehr, um „beste Variante“ zu vermeiden)
