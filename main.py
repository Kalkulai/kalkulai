...
# --- NEU: robuste Ready-Erkennung für Bestätigun für LLM1 ---
_CONFIRM_STATUS_RE = re.compile(r"status\s*:\s*best[aä]tigt", re.IGNORECASE)
_CONFIRM_TRIGGERS = [
    "ich erstelle jetzt das angebot",
    "angebot wird erstellt",
    "angebot wird jetzt erstellt",
    "ich erstelle ihnen das angebot",
    "ich erstelle das angebot",
    "mengen übernommen",
    "mengen sind übernommen",
...
def api_chat(payload: Dict[str, str] = Body(...)):
    ...
    # READY wenn:
    # - LLM1 im Maschinenanhang "status: bestätigt" liefert ODER
    # - typische Bestätigungs-/Erstellungsformulierung enthalten ist
    ready = bool(_CONFIRM_STATUS_RE.search(t)) or any(k in t for k in _CONFIRM_TRIGGERS)

    # (Optional) Backward-Compat: Falls dein Modell im Bestätigungsmodus trotzdem "Schätzung" schreibt,
    # aber bereits ein Maschinenanhang vorhanden ist, trotzdem ready.
    if not ready and ("materialien:" in t and "- name=" in t and "status:" in t):
        ready = True
